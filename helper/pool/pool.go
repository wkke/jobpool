...
// Conn is a pooled connection to a Jobpool server
type Conn struct {
	refCount    int32
	shouldClose int32
...
func (c *Conn) getRPCClient() (*StreamClient, error) {
	...
	if _, err := stream.Write([]byte{byte(RpcApp)}); err != nil {
		stream.Close()
		return nil, err
	}
...
// Jobpool servers. This is used to reduce the latency of
// RPC requests between servers. It is only used to pool
// connections in the rpcJobpool mode. Raft connections
...
func NewPool(logger hclog.Logger, maxTime time.Duration, maxStreams int) *ConnPool {
	pool := &ConnPool{
