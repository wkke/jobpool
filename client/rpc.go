...
import (
	...
	inmem "yunli.com/jobpool/helper/codec"
...
// RPC is used to forward an RPC call to a jobpool server, or fail if no servers.
...
TRY:
	...
	timer, cancel := helper.NewSafeTimer(helper.RandomStagger(c.config.RPCHoldTimeout / constant.JitterFraction))
	defer cancel()
...
// handleAppConn is used to handle a single Jobpool RPC connection.
func (c *Client) handleAppConn(conn net.Conn) {
	defer conn.Close()
