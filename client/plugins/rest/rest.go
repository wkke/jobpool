...
func NewPlugin(ctx context.Context, logger hclog.Logger) plugins.PluginRunner {
...
func NewDispatcher(ctx context.Context, logger hclog.Logger) plugins.PluginJobDispatcher {
	restClient := resty.New()
	restClient.
		// Set retry count to non zero to enable retries
		SetRetryCount(3).
...
func (p Plugin) StartSynchroJob(request *plugins.JobRequest) (*plugins.JobResult, error) {
	...
	if request == nil || request.Setting == nil {
		return nil, structs.NewErr1001Blank("请求参数")
	}
	var restSetting structs.RestSetting
	err := json.Unmarshal(request.Setting, &restSetting)
	if err != nil {
		p.logger.Warn("un marshal setting config failed", "err", err)
	...
	if err != nil {
		p.logger.Warn("send request to url failed", "url", restSetting.Url, "error", err)
		result.State = constant.JobStatusFailed
	...
	if resp.StatusCode() == 200 {
		...
		if resp.Body() != nil {
			if err := json.Unmarshal(resp.Body(), &respDto); err != nil {
				p.logger.Warn("unmarshal body error ", "body", string(resp.Body()), "error", err)
				result.State = constant.JobStatusRunning
			} else {
				if respDto.Code == 200 {
					result.State = constant.JobStatusRunning
				} else if respDto.Error != "" {
					p.logger.Warn("request send error ", "body", string(resp.Body()), "error", respDto.Error)
					result.State = constant.JobStatusFailed
...
func (p Plugin) sendRequest(restSetting *structs.RestSetting) (*resty.Response, error) {
...
func (p Plugin) DispatchSkipJob(planId string, jobId string, parameter []byte) error {
	...
	body := fmt.Sprintf(`{"planId": "%s", "jobId":"%s", "status": "%s", "message":"same job is runnint, skipped"}`, planId, jobId, constant.JobStatusSkipped)
	...
	if resp.StatusCode() == 200 {
		p.logger.Debug("dispatch no slot job success", "planId", planId)
	} else {
		p.logger.Debug("dispatch no slot job failed", "planId", planId)
	}
	return nil
}
...
func (p Plugin) DispatchNoSlotJob(planId string, jobId string, parameter []byte) error {
	...
	if resp.StatusCode() == 200 {
		p.logger.Debug("dispatch no slot job success", "planId", planId, "jobId", jobId)
	} else {
		p.logger.Debug("dispatch no slot job failed", "planId", planId, "jobId", jobId)
	}
	return nil
}
