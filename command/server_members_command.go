...
func (c *ServerMembersCommand) Help() string {
	helpText := `
Usage: jobpool server members [options]

  Display a list of the known servers and their status. Only Jobpool servers are
...
func standardOutput(mem []*client.AgentMember, leaders map[string]string) []string {
...
func verboseOutput(mem []*client.AgentMember, leaders map[string]string) []string {
	// Format the members list
	members := make([]string, len(mem)+1)
...
// leader.
func regionLeaders(client *client.Client, mem []*client.AgentMember) (map[string]string, error) {
...
func isLeader(member *client.AgentMember, leaders map[string]string) bool {
