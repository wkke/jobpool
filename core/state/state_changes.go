...
func (c *changeTrackerDB) WriteTxn(idx uint64) *txn {
	t := &txn{
		...
		Index:   idx,
		publish: c.publish,
		msgType: constant.IgnoreUnknownTypeFlag, // The zero value of structs.MessageType is noderegistration.
	}
	t.Txn.TrackChanges()
	return t
}

func (c *changeTrackerDB) WriteTxnMsgT(msgType constant.MessageType, idx uint64) *txn {
	t := &txn{
		msgType: msgType,
		Txn:     c.memdb.Txn(true),
		Index:   idx,
...
func (c *changeTrackerDB) publish(changes Changes) (*structs.Events, error) {
	...
	if events != nil {
		//strInfo, _ := json.Marshal(events)
		//fmt.Println(fmt.Printf("the event to be publish is : %s", strInfo))
...
func (tx *txn) MsgType() constant.MessageType {
	return tx.msgType
}
