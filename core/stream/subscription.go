...
func filter(req *SubscribeRequest, events []structs.Event) []structs.Event {
	...
	allTopicKeys := req.Topics[constant.TopicAll]

	// Return all events if subscribed to all namespaces and all topics
	if req.Namespace == "*" && len(allTopicKeys) == 1 && allTopicKeys[0] == string(constant.TopicAll) {
		return events
	}
	...
	for _, event := range events {
		...
		// *[*] always matches
		if len(allTopicKeys) == 1 && allTopicKeys[0] == string(constant.TopicAll) {
		...
		if len(keys) == 1 && keys[0] == string(constant.TopicAll) {
			result = append(result, event)
			continue
		}
