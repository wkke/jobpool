...
// nodeConnState is used to track connection information about a Jobpool Client.
...
func (s *Server) serverWithNodeConn(nodeID, region string) (*serverParts, error) {
	...
	for _, server := range targets {
		...
		// Make the RPC
		var resp dto.NodeConnQueryResponse
...
func NodeRpc(session *yamux.Session, method string, args, reply interface{}) error {
	...
	// Write the RpcApp byte to set the mode
	if _, err := stream.Write([]byte{byte(pool.RpcApp)}); err != nil {
		stream.Close()
