...
func handleJob(snap *state.StateSnapshot, job *structs.Job, allocs []*structs.Allocation, lastHandledIndex uint64) (*jobResult, error) {
	r := newJobResult()
	if err := handleTaskGroup(snap, false, job, allocs, lastHandledIndex, r); err != nil {
		return nil, fmt.Errorf("drain for job %q failed: %v", job.Name, err)
	}
	return r, nil
}
...
func handleTaskGroup(snap *state.StateSnapshot, batch bool, tg *structs.Job,
...
func (w *drainingJobWatcher) getJobAllocsImpl(ws memdb.WatchSet, state *state.StateStore) (interface{}, uint64, error) {
	...
	for jns := range draining {
		allocs, err := state.AllocsByJob(ws, jns.Namespace, jns.ID)
		if err != nil {
