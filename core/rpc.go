...
func (r *rpcHandler) listen(ctx context.Context) {
	...
	for {
		...
		metrics.IncrCounter([]string{constant.JobPoolName, "rpc", "accept_conn"}, 1)
	}
}
...
// Jobpool type RPC connection and invoke the correct handler
//
...
func (r *rpcHandler) handleConn(ctx context.Context, conn net.Conn, rpcCtx *RPCContext) {
	...
	case pool.RpcApp:
		...
		r.handleAppConn(ctx, conn, server)
	...
	case pool.RpcRaft:
		metrics.IncrCounter([]string{constant.JobPoolName, "rpc", "raft_handoff"}, 1)
...
func (r *rpcHandler) handleMultiplex(ctx context.Context, conn net.Conn, rpcCtx *RPCContext) {
	...
	for {
		...
		go r.handleAppConn(ctx, sub, rpcServer)
	}
}

// handleJobpoolConn is used to service a single Jobpool RPC connection
func (r *rpcHandler) handleAppConn(ctx context.Context, conn net.Conn, server *rpc.Server) {
	defer conn.Close()
	...
	for {
		...
		if err := server.ServeRequest(rpcCodec); err != nil {
			if err != io.EOF && !strings.Contains(err.Error(), "closed") {
				...
				metrics.IncrCounter([]string{constant.JobPoolName, "rpc", "request_error"}, 1)
			}
			return
		}
		metrics.IncrCounter([]string{constant.JobPoolName, "rpc", "request"}, 1)
	}
}

// handleStreamingConn is used to handle a single Streaming Jobpool RPC connection.
func (r *rpcHandler) handleStreamingConn(conn net.Conn) {
	...
	if err := decoder.Decode(&header); err != nil {
		if err != io.EOF && !strings.Contains(err.Error(), "closed") {
			...
			metrics.IncrCounter([]string{constant.JobPoolName, "streaming_rpc", "request_error"}, 1)
		}
	...
	// Invoke the handler
	metrics.IncrCounter([]string{constant.JobPoolName, "streaming_rpc", "request"}, 1)
	handler(conn)
}
...
CHECK_LEADER:
	...
	if time.Since(firstCheck) < r.config.RPCHoldTimeout {
		jitter := RandomStagger(r.config.RPCHoldTimeout / constant.JitterFraction)
		select {
...
func (r *rpcHandler) forwardRegion(region, method string, args interface{}, reply interface{}) error {
	...
	// Forward to remote Jobpool
	metrics.IncrCounter([]string{constant.JobPoolName, "rpc", "cross-region", region}, 1)
...
func (r *rpcHandler) getServer(region, serverID string) (*serverParts, error) {
	...
	return nil, fmt.Errorf("unknown Jobpool server %s", serverID)
}
...
func (s *Server) raftApplyFuture(t constant.MessageType, msg interface{}) (raft.ApplyFuture, error) {
	buf, err := structs.Encode(t, msg)
	info := string(buf)
	s.logger.Trace("raft apply future log is :", info)
	if err != nil {
...
type raftApplyFn func(t constant.MessageType, msg interface{}) (interface{}, uint64, error)

// raftApply is used to encode a message, run it through raft, and return
// the FSM response along with any errors
func (s *Server) raftApply(t constant.MessageType, msg interface{}) (interface{}, uint64, error) {
...
func (r *rpcHandler) setQueryMeta(m *dto.QueryMeta) {
	if r.IsLeader() {
		m.LastContact = 0
		m.KnownLeader = true
	} else {
...
type blockingOptions struct {
	queryOpts *dto.QueryOptions
	queryMeta *dto.QueryMeta
	run       queryFn
}
...
func (r *rpcHandler) blockingRPC(opts *blockingOptions) error {
	...
	opts.queryOpts.MaxQueryTime += RandomStagger(opts.queryOpts.MaxQueryTime / constant.JitterFraction)
...
RUN_QUERY:
	...
	metrics.IncrCounter([]string{constant.JobPoolName, "rpc", "query"}, 1)
